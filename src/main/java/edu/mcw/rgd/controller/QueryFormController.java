...
//						synonym.getName().toLowerCase().replaceAll("\\s", " AND ")).append(")");
	...
	private String guessConcept(String termStr, String termCat) {
		...
		try {
			URI uri = new URI("https",BasicUtils.hostname, "/OntoSolr/select", "q=cat:(RDO RGD_GENE) OR term_str:(\""
	...
	private List<String> guessConcept1(String termStr, String termCat) {
		...
		try {
			URI uri = new URI("https","localhost:8080", "/OntoSolr/select", "q=cat:(RDO RGD_GENE) OR term_str:(\""
					+termStr+"\")^50 OR synonym_str:(\"" + termStr + "\")^45 OR ("
...
//					(termCat == null ? "":"&fq=cat:"+termCat) + "&wt=csv&fl=def&csv.header=false&csv.separator=|", null);
			URI uri = new URI("https","localhost:8080", "/OntoSolr/select", "q=(cat:(RDO^20 OR UMLS^15 OR HP^10 OR MP^2) AND (term_str:(\""
		+termStr+"\")^50 OR synonym_str:(\"" + termStr + "\")^45 OR term_en:("
		...
		for (FieldQueryCondition fqc : queryString.getqFieldConditions()) {
			if (fqc.getFieldValue() != null && fqc.getFieldValue().length() > 0) {
				...
				if (fqc.getFieldName().equals("rgd_gene_term")) {
					...
					try {
						URI uri = new URI("https",BasicUtils.hostname, "/OntoSolr/select", "q=mendelian+OR+inheritance+OR+familial+OR+genetic+OR+ancestral+OR+patrimonial+OR+familial&fq=cat:\"RDO\"&fl=id&wt=velocity&v.template=idstring&rows=1000000", null);
