...
//			solrQString = solrQString.trim() + "&sort=" + sortString;
		...
		if(queryString.getqSource()!=null) {
			if (queryString.getqSource().equalsIgnoreCase("pubmed")) {
				...
				model.addAttribute("source", "https://dev.rgd.mcw.edu/solr/preprintSolr");
			}
		}else{
...
//						synonym.getName().toLowerCase().replaceAll("\\s", " AND ")).append(")");
	...
	private String guessConcept(String termStr, String termCat) {
		...
		try {
			URI uri = new URI("https","dev.rgd.mcw.edu", "/solr/OntoSolr/select", "q=cat:(RDO RGD_GENE) OR term_str:(\""
